...
\begin{description}[noitemsep,align=right]
  ...
  \item[Input] User-specified query from the following list:
    ...
    QPMA (Query for Match's Attendance)
    
    QGMS (Query for a Given Match's Stadium)
...
\begin{itemize}
	...
	\item QPMA (Query for Match's Attendance)
	\item QGMS (Query for a Given Match's Stadium)
